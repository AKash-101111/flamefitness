...
const Footer = () => {
    ...
    return (
        <footer className="w-full rounded-t-[60px] md:rounded-t-[120px] relative overflow-hidden bg-[#050505] text-white pt-24 pb-12 border-t border-[var(--primary)]/10 shadow-[0_-20px_100px_rgba(0,0,0,0.8)]">
            ...
            <div className="max-w-7xl mx-auto px-6 lg:px-12 relative z-10">
                ...
                <div className="flex flex-col items-center mb-20 text-center">
                    ...
                    <p className="max-w-3xl text-lg md:text-xl leading-relaxed glass-card p-8 bg-white/[0.02] border-[var(--primary)]/10 montserrat italic text-white/60">
                        "Elevate your strength and performance. Join <span className="text-[var(--primary)] font-bold">Flame Fitness Studio</span> to transform your body, mind, and fitness journey with expert guidance."
                    </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-16 md:gap-8">
                    ...
                    {sections.map((section, idx) => (
                        <div key={idx} className="flex flex-col">
                            <h3 className="text-white text-base font-black tracking-[0.25em] uppercase mb-10 league-spartan border-b border-[var(--primary)]/20 pb-2 w-fit">
                                {section.title}
                            </h3>
                            <ul className="flex flex-col gap-5">
                                {section.links.map((link, i) => (
                                    <li key={i}>
                                        <Link 
                                            to={link.path} 
                                            className="text-white/40 hover:text-[var(--primary)] transition-all duration-300 text-sm md:text-base poiret hover:pl-2"
                                        >
                                            {link.name}
                                        </Link>
                                    </li>
                ...
                <div className="mt-24 pt-10 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-6 text-center md:text-left">
                    <p className="text-xs md:text-sm tracking-widest montserrat text-white/30">
                        © {currentYear} <span className="text-[var(--primary)] font-black">Flame Fitness Studio</span>. All Rights Reserved. Made with Passion 🇮🇳
                    </p>
                    ...
                    <div className="flex flex-wrap justify-center gap-4">
                        {socialLinks.map((social, idx) => (
                            <motion.a
                                ...
                                whileTap={{ scale: 0.9 }}
                                className="w-14 h-14 flex items-center justify-center rounded-full bg-white/5 border border-white/10 text-white/70 hover:bg-[var(--primary)] hover:border-[var(--primary)] hover:text-[#050505] transition-all duration-300 shadow-xl"
